...
int main(void) {
  ...
  /* Get my process rank */
   ...
   local_a = BLOCK_LOW(my_rank,n,comm_sz);  //Finds the lower point of the curve section
   local_b = BLOCK_HIGH(my_rank,n,comm_sz); //Finds the higher point of the curve section 
   local_n = local_b - local_a + 1;         //Finds the total number of points in the section
   ...
   if (my_rank == 0) {
      ...
      printf("of the integral from %f to %f = %.15e\n",
          a, b, total_int);
	  printf("The Error value is %.16f\n",fabs(total_int - PI25DT));
